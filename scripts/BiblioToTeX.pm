...
{
	print	"\t-e      :\t create an example TeX file to test the TeX output, and a script to run latex\n";
}
...
{
	my( $fname, $hasBiblio, $run_script ) = @_;

	my $script_name = "cleanup-$fname.sh";
	open FILE, "> $script_name"
	  or die "Error: Could not open:\t $script_name\n";

	my @suffix = qw/ tex log aux /;
	push( @suffix, qw/ blg bbl / )	if( $hasBiblio );
	foreach my $suffix ( @suffix )
	{
		print FILE "rm ${fname}.${suffix}\n";
	}
...
sub create_exampleTeX_Script
{
	...
	my $fname = "$tableTeX-list.tex";
	   $fname = "$tableTeX-deluxe-list.tex"
	...
	&create_cleanup_script( $fname, $hasBiblio, $run_script );
}
