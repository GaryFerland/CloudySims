...
# Set ADS API token (Replace with your own ADS token)-This is Dehghanian's personal token. If does not work, you need to replace with your own. Can be acquired from ADS when logged in

ads.config.token = "f0alCixThOvlEx30gHn3lYXCRGHLkFAC8GKbv9VR"
...
def fetch_hazy_citations(bibcodes):
    ...
    total_citations_check = 0

    for bibcode in bibcodes:
        try:
            metrics = ads.MetricsQuery(bibcodes=[bibcode]).execute()
            total_citations = metrics.get("citation stats", {}).get("total number of citations", 0)
            total_citations_check += total_citations
...
version_colors = ["gray", "orange", "green", "red", "blue"]
...
# **Step 10: Overlay Refereed Citations as a Line Plot**
refereed_counts = [refereed_citations_by_year.get(year, 0) for year in years]
plt.plot(years, refereed_counts, label="Refereed Papers", color="black", marker="o")

# **Step 11: Add Labels, Legend, and Title**
plt.xlabel("Year")
