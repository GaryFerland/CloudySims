...
static long int ndimOpacityStack = 4200000L;
...
void OpacityCreateAll()
{
	...
	if( opac.nOpacTot + iso_sp[ipH_LIKE][ipHYDROGEN].fb[ipH1s].ipIsoLevNIonCon + rfield.nflux_with_check > ndimOpacityStack )
		opacity_more_memory();
	...
	{
		opac.OpacStack[i-1+opac.ipRayScat] = rayleh(rfield.anu(i));
	}
	...
	{
		opac.OpacStack[i-1+opac.iopcom] = SIGMA_THOMSON;
		/*fprintf(ioQQQ,"%.3e\t%.3e\n", 
			rfield.anu(i)*EVRYD , opac.OpacStack[i-1+opac.iopcom] );*/
	}
	...
	for( i=opac.ipCKshell; i < rfield.nflux_with_check; i++ )
	{
		...
		opac.OpacStack[i-1+opac.iopcom] = SIGMA_THOMSON*3.e0/4.e0*((1.e0 + 
	...
	/* >>chng 02 may 08, by Ryan.  Added this and other checks for allotted memory.	*/
	if( opac.nOpacTot + 3*rfield.nflux_with_check - opac.ippr + iso_sp[ipHE_LIKE][ipHELIUM].fb[0].ipIsoLevNIonCon - hmi.iphmin + 2 > ndimOpacityStack )
		opacity_more_memory();
	...
	for( i=opac.ippr-1; i < rfield.nflux_with_check; i++ )
	{
		...
		/*fprintf(ioQQQ,"%.3e\t%.3e\n", 
			rfield.anu(i)*EVRYD , opac.OpacStack[i-opac.ippr+opac.ioppr] );*/
	}
	...
	for( i=0; i < rfield.nflux_with_check; i++ )
	{
		...
		opac.OpacStack[i-1+opac.ipBrems] = FREE_FREE_ABS * 1e30 / POW3(rfield.anu(i));
	}
	opac.nOpacTot += rfield.nflux_with_check - 1 + 1;
	...
	for( i=0; i < rfield.nflux_with_check; i++ )
	{
		...
		opac.OpacStack[i-1+opac.iphmra] = 0.1175*rfield.anusqrt(i);
	}
	opac.nOpacTot += rfield.nflux_with_check - 1 + 1;
	...
	for( diatom_iter diatom = diatoms.begin(); diatom != diatoms.end(); ++diatom )
	{
		if( opac.nOpacTot + rfield.nflux_with_check - (*diatom)->ip_photo_opac_thresh + 1 > ndimOpacityStack )
			opacity_more_memory();
	...
	/* H2+ H2P h2plus photoabsorption */
	{
		...
		if( opac.nOpacTot + opac.ih2pnt[1] - opac.ih2pnt[0] + 1 > ndimOpacityStack )
			opacity_more_memory();
		...
		if( opac.nOpacTot + opac.ih2pnt_ex[1] - opac.ih2pnt_ex[0] + 1 > ndimOpacityStack )
			opacity_more_memory();
	...
	/* >>chng 02 may 08, by Ryan.  Added this and other checks for allotted memory.	*/
	if( opac.nOpacTot + rfield.nflux_with_check - iso_sp[ipHE_LIKE][ipHELIUM].fb[0].ipIsoLevNIonCon + 1 > ndimOpacityStack )
		opacity_more_memory();
	...
	if( !( t_version::Inst().lgRelease || t_version::Inst().lgReleaseBranch ) && lgRealloc && prt.lgPrintTime )
		fprintf(ioQQQ,
		" Please consider revising ndimOpacityStack=%li in OpacityCreateAll, a total of %li cells were needed.\n\n" ,
		ndimOpacityStack , opac.nOpacTot);
	return;
}
/*OpacityCreatePowerLaw generate array of cross sections using a simple power law fit */
STATIC void OpacityCreatePowerLaw(
...
STATIC void opacity_more_memory(void)
{
	...
	/* double size */
	ndimOpacityStack *= 2;
	double *newptr = (double *)MALLOC( (size_t)ndimOpacityStack*sizeof(double) );
	memcpy( newptr, opac.OpacStack, ndimOpacityStack*sizeof(double)/2 );
	free( opac.OpacStack );
	opac.OpacStack = newptr;
	/* only print on development branch - if we must resize often then should consider
	 * changing default.  The comment in the release version causes concern.
	 */
