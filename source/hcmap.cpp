			...
			const char *chType)
{
	...
	if( hcmap.nMapAlloc==0 )
	{
		...
		/* now make the space */
		hcmap.temap = (double*)MALLOC( sizeof(double)*(size_t)(hcmap.nMapStep+4) );
		if( hcmap.temap == NULL )
		{ 
			printf( " not enough memory to allocate hcmap.temap in map_do\n" );
			cdEXIT(EXIT_FAILURE);
		}
		hcmap.cmap = (double*)MALLOC( sizeof(double)*(size_t)(hcmap.nMapStep+4) );
		if( hcmap.cmap == NULL )
		{ 
			printf( " not enough memory to allocate hcmap.cmap in map_do\n" );
			cdEXIT(EXIT_FAILURE);
		}
		hcmap.hmap = (double*)MALLOC( sizeof(double)*(size_t)(hcmap.nMapStep+4) );
		if( hcmap.hmap == NULL )
		{ 
