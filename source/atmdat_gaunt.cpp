...
void t_gaunt::p_read_table(const char* fnam, long nelem)
{
	...
	fstream io;
	open_data( io, fnam1.c_str(), mode_r );
	string line;
	// read magic number
	getline( io, line );
...
void t_gaunt::p_gauntff_vec_sub(long Z, double Te, const double anulog10[], long nmin, long nmax)
{
	...
	vector< realnum, allocator_avx<realnum> >& gff = p_gff_ion[Z].gff;
...
void t_gaunt::brems_opac(long ion, double Te, double abun, double arr[])
{
		...
		for( long loggamma2=-500; loggamma2 <= 900; loggamma2++ )
		{
			    ...
			    ERyd > rfield.emm() && ERyd < rfield.egamry() )
			{
				...
				if( i < 2 )
					...
					if( abs((gaunt[2]+gaunt[0])/(2.*gaunt[1]) - 1.) > toler ) {
						fprintf( ioQQQ, " PROBLEM DISASTER cont_gaunt_calc discontinuity in"
							 " gam2 direction at gam2 = %e u = %e\n", gam2[i], u );
						fprintf( ioQQQ, "     gam2 = %e gaunt = %e\n", gam2[0], gaunt[0] );
						fprintf( ioQQQ, "     gam2 = %e gaunt = %e\n", gam2[1], gaunt[1] );
						fprintf( ioQQQ, "     gam2 = %e gaunt = %e\n", gam2[2], gaunt[2] );
		...
		for( long logu=-1300; logu <= 1200; logu++ )
		{
			    ...
			    ERyd > rfield.emm() && ERyd < rfield.egamry() )
			{
				...
				if( i < 2 )
					...
					if( abs((gaunt[2]+gaunt[0])/(2.*gaunt[1]) - 1.) > toler ) {
						fprintf( ioQQQ, " PROBLEM DISASTER cont_gaunt_calc discontinuity in"
							 " u direction at gam2 = %e u = %e\n", gam2, u[2] );
						fprintf( ioQQQ, "     u = %e gaunt = %e\n", u[0], gaunt[0] );
						fprintf( ioQQQ, "     u = %e gaunt = %e\n", u[1], gaunt[1] );
						fprintf( ioQQQ, "     u = %e gaunt = %e\n", u[2], gaunt[2] );
						exit_status = ES_FAILURE;
						i = 0;
