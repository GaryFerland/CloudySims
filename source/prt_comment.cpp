...
void PrtComment(void)
{
	...
	if( atmdat.lgdBaseSourceExists[ipOXYGEN][2] )
	{
		...
		if( cdLine("Blnd",4363,&t4363,&absint)<=0 )
		{
		...
		if( cdLine("O  3",wlAirVac(4363.21),&c4363,&absint)<=0 )
		{
			fprintf( ioQQQ, " PrtComment could not find collisional O III 4363 with cdLine, assuming intensity of zero.\n" );
			c4363 = 0;
			absint = -37.;
		}
	...
	if( cdLine("H  1",1215.67f,&relhm,&absint)<=0 )
	{
	...
	*totwid = 0.;
		...
		iso_sp[ipH_LIKE][ipHYDROGEN].nCollapsed_local >= 6 )
	{
			...
			iso_sp[ipH_LIKE][ipHYDROGEN].trans(ip6p,ipH2s).Emis().TauIn() >   0. )
		{
			...
			conca = sqrt(6.1e-5*ca2_3969_TauIn);
			conalog = log((double)ca2_3969_TauIn);
			...
			conalog = sqrt(MAX2(1.,conalog));
			conhe = sqrt(1.7e-6*iso_sp[ipH_LIKE][ipHYDROGEN].trans(ip6p,ipH2s).Emis().TauIn());
