...
#if 0
STATIC double iso_get_collision_strength_collapsed_to_collapsed( long ipISO, long nelem, long ipCollider,
					long nHi, double IP_Ryd_Hi,
					long nLo, double IP_Ryd_Lo,
					double Aul, double tauLo, double EnerWN, double EnerErg );
#endif
STATIC double iso_get_collision_strength_collapsed_to_resolved( long ipISO, long nelem, long ipCollider,
					long nHi, double IP_Ryd_Hi,
					long nLo, long lLo, long sLo, long jLo, long gLo, double IP_Ryd_Lo,
					double Aul, double tauLo, double EnerWN, double EnerErg );
STATIC double iso_get_collision_strength_resolved( long ipISO, long nelem, long ipCollider,
...
STATIC double iso_get_collision_strength_collapsed_to_collapsed_fast( long ipISO, long nelem, long ipCollider,
					long nHi, long gHi, double IP_Ryd_Hi,
					long nLo, double IP_Ryd_Lo,
					double tauLo, double EnerWN, double EnerErg );
...
STATIC double iso_get_collision_strength( long ipISO, long nelem, long ipCollider, long ipHi, long ipLo )
{
	...
	if( nHi==nLo && !iso_ctrl.lgColl_l_mixing[ipISO] )
		...
		cs = 0.;
	else if( nHi > sp->n_HighestResolved_max && nLo > sp->n_HighestResolved_max )
	{
		ASSERT( ipLo >= sp->numLevels_max - sp->nCollapsed_max );
#if 1
		cs = iso_get_collision_strength_collapsed_to_collapsed_fast( ipISO, nelem, ipCollider,
					nHi,gHi,IP_Ryd_Hi,
					nLo, IP_Ryd_Lo,
					tauLo, EnerWN, EnerErg );
#else
		cs = iso_get_collision_strength_collapsed_to_collapsed( ipISO, nelem, ipCollider,
					nHi, IP_Ryd_Hi,
					nLo, IP_Ryd_Lo,
					Aul, tauLo, EnerWN, EnerErg );
#endif
	}
	else if( nHi > sp->n_HighestResolved_max && nLo <= sp->n_HighestResolved_max )
	{
		ASSERT( ipLo < sp->numLevels_max - sp->nCollapsed_max );
		cs = iso_get_collision_strength_collapsed_to_resolved( ipISO, nelem, ipCollider, 
					nHi, IP_Ryd_Hi,
	...
	{
		const char *where = "      ";
		cs = iso_get_collision_strength_resolved( ipISO, nelem, ipCollider, 
...
STATIC double iso_get_collision_strength_collapsed_to_collapsed_fast( long ipISO, long nelem, long ipCollider,
					...
					double tauLo, double EnerWN, double EnerErg )
{
	DEBUG_ENTRY( "iso_get_collision_strength_collapsed_to_collapsed_fast()" );

	t_iso_sp* sp = &iso_sp[ipISO][nelem];
	ASSERT( nLo > sp->n_HighestResolved_max );
	ASSERT( nLo > 2 );
	...
	for( long lLo = 0; lLo < nLo; ++lLo )
	{
		...
		if( ipISO==ipHE_LIKE )
			sLo=1;
		long sHi = sLo;
		// NB - This is the index we would have if all n were resolved. 2nd dim of CachedAs is indexed by this.	
		long ipLoRes = sp->IndexIfAllResolved[nLo][lLo][sLo];
		long g_nHi;

		if( lLo!= 0 )
		{
			g_nHi = (2*(lLo-1)+1)*sHi;
			Aul_total += g_nHi * sp->CachedAs[ nHi-sp->n_HighestResolved_max-1 ][ ipLoRes ][1];
			++nAul;
		}

		g_nHi = (2*(lLo+1)+1)*sHi;
		Aul_total += g_nHi * sp->CachedAs[ nHi-sp->n_HighestResolved_max-1 ][ ipLoRes ][0];
		++nAul;

		// Account for triplet counterparts.
		if( ipISO==ipHE_LIKE )
		{
			sLo=3;
			sHi=sLo;
			ipLoRes = sp->IndexIfAllResolved[nLo][lLo][sLo];
			if( lLo!= 0 )
			{
				g_nHi = (2*(lLo-1)+1)*sHi;
				Aul_total += g_nHi * sp->CachedAs[ nHi-sp->n_HighestResolved_max-1 ][ ipLoRes ][1];
	...
	{
		g_nLo = (4 * POW2(nLo));
		factor = (2 * nHi - 2) * g_nLo;
	}
 	else
		TotalInsanity();
	ASSERT( factor > 0 );
	const char *where = "      ";
	if (opac.lgCaseB_HummerStorey || (ipISO == ipH_LIKE && nelem != ipHYDROGEN ) )
		cs = iso_get_collision_strength_resolved( ipISO, nelem, ipCollider,
				nHi, -1, -1, -1, 1, IP_Ryd_Hi,
				nLo, -1, -1, -1, 1, IP_Ryd_Lo,
				Aul_total/nAul, tauLo, EnerWN, EnerErg, &where );
	else
	{
		cs = iso_get_collision_strength_resolved( ipISO, nelem, ipCollider,
				nHi, -1, -1, -1, 1, IP_Ryd_Hi,
		...
		if ( strcmp(where,"Vriens") == 0 || strcmp(where,"PR78  ") == 0)
			cs *= (double)gHi;
		else
		{
			cs *=nAul;
			cs += factor * iso_get_collision_strength_resolved( ipISO, nelem, ipCollider,
					nHi, -1, -1, -1, 1, IP_Ryd_Hi,
					nLo, -1, -1, -1, 1, IP_Ryd_Lo,
					iso_ctrl.SmallA, tauLo, EnerWN, EnerErg, &where );
		}
	}
...
STATIC double iso_get_collision_strength_collapsed_to_collapsed( long ipISO, long nelem, long ipCollider,
					...
					double Aul, double tauLo, double EnerWN, double EnerErg )
{
	DEBUG_ENTRY( "iso_get_collision_strength_collapsed_to_collapsed()" );

	t_iso_sp* sp = &iso_sp[ipISO][nelem];
	ASSERT( nLo > sp->n_HighestResolved_max );

	vector<long> ang_mom_lo;
	for( long i = 0; i < nLo; ++i )
		ang_mom_lo.push_back( i );

	double cs = 0.;
	for( vector<long>::iterator itLo = ang_mom_lo.begin(); itLo != ang_mom_lo.end(); ++itLo )
	{
		long lLo = *itLo;
		long sLo = 2;
		if( ipISO==ipHE_LIKE )
		...
		long jLo = -1;
		long gLo = (2*lLo+1) * sLo;
		double cs1 = iso_get_collision_strength_collapsed_to_resolved( ipISO, nelem, ipCollider, 
					nHi, IP_Ryd_Hi,
					nLo, lLo, sLo, jLo, gLo, IP_Ryd_Lo,
					Aul, tauLo, EnerWN, EnerErg );
		cs += cs1;

		// Now do other spin for He-like
		if( ipISO==ipHE_LIKE )
		{
			sLo = 3;
			long gLo = (2*lLo+1) * sLo;
...
STATIC double iso_get_collision_strength_collapsed_to_resolved( long ipISO, long nelem, long ipCollider,
					...
					double Aul, double tauLo, double EnerWN, double EnerErg )
{
	DEBUG_ENTRY( "iso_get_collision_strength_collapsed_to_resolved()" );
	...
	if( lLo!= 0 )
	{
		lHi = lLo-1;
		gHi = (2*lHi+1) * sHi;
		Aul = sp->CachedAs[ nHi-sp->n_HighestResolved_max-1 ][ ipLoRes ][1];
		cs_less_1 = iso_get_collision_strength_resolved( ipISO, nelem, ipCollider, 
				nHi, lHi, sHi, jHi, gHi, IP_Ryd_Hi,
				nLo, lLo, sLo, jLo, gLo, IP_Ryd_Lo,
				Aul, tauLo, EnerWN, EnerErg, &where );
	}
	...
	lHi = lLo+1;
	gHi = (2*lHi+1) * sHi;
	Aul = sp->CachedAs[ nHi-sp->n_HighestResolved_max-1 ][ ipLoRes ][0];
	double cs_plus_1 = iso_get_collision_strength_resolved( ipISO, nelem, ipCollider, 
			nHi, lHi, sHi, jHi, gHi, IP_Ryd_Hi,
			nLo, lLo, sLo, jLo, gLo, IP_Ryd_Lo,
			Aul, tauLo, EnerWN, EnerErg, &where );
	...
	if( lLo-2 >= 0 )
		...
		// This can't be true because nHi is asserted >= 4 above.
		TotalInsanity();

	gHi = 1;
	Aul = iso_ctrl.SmallA;
	double cs_other = iso_get_collision_strength_resolved( ipISO, nelem, ipCollider, 
			nHi, lHi, sHi, jHi, gHi, IP_Ryd_Hi,
			nLo, lLo, sLo, jLo, gLo, IP_Ryd_Lo,
			Aul, tauLo, EnerWN, EnerErg, &where );
	...
	if( lLo==0 )
		factor = nHi-1;
	// There are twice as many transitions from He-like than from H-like.
	if( ipISO==ipHE_LIKE )
		factor *= 2;
	cs = cs_less_1 + cs_plus_1 + factor * cs_other;
...
STATIC double iso_get_collision_strength_resolved( long ipISO, long nelem, long ipCollider,
					...
					double Aul, double tauLo, double EnerWN, double EnerErg, const char **where )
{
	DEBUG_ENTRY( "iso_get_collision_strength_resolved()" );
