...
double ratefact( long nelem,long ipCollider)
{
	/*This routine calculates the factor to convert from collision strengths Y
	...
	factor = powpq(ELECTRON_MASS/reduced_mass_collider_system,3,2) * COLL_CONST/phycon.sqrte;
...
void iso_collide( long ipISO, long nelem )
{
			...
			for( long ipHi=1; ipHi<sp->numLevels_max; ipHi++ )
			{
				...
				for( long ipLo=0; ipLo < ipHi; ipLo++ )
				{
					for( long ipCollider = ipELECTRON; ipCollider <= ipALPHA; ipCollider++ )
					{
						...
						double rateCoef = cs_temp * ratefact(nelem,ipCollider) * overHig;
    ...
    /* Source and sink terms for the condensed matrix
     * This is only done once, under the assumption that
     * temperature updates are not that important in these terms.
     */
	if (!iso_ctrl.lgMatCondOff[ipISO][nelem] && !iso_ctrl.lgNoExtraTermCalc[ipISO][nelem])
	{
		add_equilibrium_levels(nelem,ipISO,sp->mmax );

		iso_ctrl.lgNoExtraTermCalc[ipISO][nelem] = true;
	}
...
STATIC double iso_get_collision_strength( long ipISO, long nelem, long ipCollider, long ipHi, long ipLo )
{
	...
	{
		double Aul_hydro = HydroEinstA( (double) nLo, (double) nHi );
