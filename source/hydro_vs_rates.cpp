...
double CS_VS80( long nHi, long gHi, double IP_Ryd_Hi, long nLo, long gLo, double IP_Ryd_Lo, double Aul, long nelem, long Collider, double temp )
{
	...
	{
		coll_str = hydro_vs_deexcit( (double) nHi, (double)gHi, IP_Ryd_Hi, nLo, gLo, IP_Ryd_Lo, Aul );
	}
	else
	{
...
double hydro_vs_deexcit( double nHi, double gHi, double IP_Ryd_Hi, long nLo, long gLo, double IP_Ryd_Lo, double Aul )
{
 ...
 */
double hydro_Lebedev_deexcit(long nelem, long ipISO, double nHi , long nLo, long gLo, double IP_Ryd_Lo)
{
	/*core charge*/
	double Z = (double)nelem-(double)ipISO + 1.;
	double deltan = nHi-(double)nLo;
	double npn = nHi + (double)nLo;
	double npn2 = npn*npn;
