	...
	t_phoHeat *photoHeat)
{
	...
	double help;
	photoHeat->HeatNet +=
		reduce_abc_ab( rfield.SummedCon, &opac.OpacStack[ipOffset], rfield.anuptr(), ipLoEnr, limit, &help );
	gamk_v += help;
	...
	double GamHi;
	photoHeat->HeatHiEnr =
		reduce_abc_ab( rfield.SummedCon, &opac.OpacStack[ipOffset], rfield.anuptr(), ilo, iup, &GamHi );
