...
{
	DEBUG_ENTRY( "getLevels()" );

	char chLabel_species[NCHLAB] = { 0 };
	strcpy( chLabel_species, chLabel );

	long nLevelLo = -1,
		nLevelHi = -1;
	bool lgLevels = false;

	for(char* s=chLabel_species; *s != '\0'; ++s)
	{
		if (*s == '[')
		{
			lgLevels = true;
			*s = '\0';
			...
			if (*s != ':')
			{
				nLevelLo = strtol(s,&t,10);
				if (nLevelLo <= 0)
				{
					fprintf(ioQQQ,"PROBLEM: In request for species \"%s\"\n",chLabel);
					fprintf(ioQQQ,"Excitation levels must be at >= 1\n");
					cdEXIT(EXIT_FAILURE);
				}				
				s = t;
			}
			...
			{
				fprintf(ioQQQ,"PROBLEM: In request for species \"%s\"\n",chLabel);
				fprintf(ioQQQ,"Excitation levels must be in balanced brackets\n");
				cdEXIT(EXIT_FAILURE);
			}
			break;
		}
	...
	const molezone *sp = findspecieslocal(chLabel_species);
		...
		else if ( size_t(nLevelLo) > sp->levels->size() || size_t(nLevelHi) > sp->levels->size() )
		{
			if (lgValidate)
			{
				fprintf(ioQQQ,"PROBLEM: Request for level \"%s\", but species only has %lu levels\n",
					chLabel,(unsigned long)sp->levels->size());
				cdEXIT(EXIT_FAILURE);
			}
		}
		else
