...
STATIC void grouped_elems(const double bvec[], double mole_elems[])
{
	...
	{
		mole_elems[j] = 0.;
		nuclide_to_index[nuclide_list[j].get_ptr()] = j;
	}
	for( long i=0; i < mole_global.num_compacted; i++ )
	{
		...
		{
			mole_elems[nuclide_to_index[el->first.get_ptr()]] += bvec[i]*el->second;
		}
	}
}
		...
		dynamics.Rate != 0.0 )
	{
		...
		for( long i=0;i<mole_global.num_calc;i++)
		{
			...
			else if (mole_global.list[i]->charge == 0)
			{
				...
				const count_ptr<chem_nuclide>& atom = mole_global.list[i]->nNuclide.begin()->first;
