...
void PrtOneMonitor( FILE *ioMONITOR, const char* chAssertType, const string chAssertLineLabel, 
...
void ParseMonitorResults(Parser &p)
{
	...
	if( p.nMatch( "GRID" ) )
	{
		...
		bool lgEOF;
		input_readvector( chLabel.c_str(), get_ptr(AssertVector), optimize.nOptimiz+1, &lgEOF );
		if( lgEOF )
	...
	else if( p.nMatch(" LINE " ) )
	{
		...
		else if( p.nMatch( "CASE" ) && p.nMatch( " B ") )
		{
			...
			strcpy( chAssertType[nAsserts] , "Lb" );
		...
		blend_iterator b=blends.find(chLabel);
		if(0 && b != blends.end())
		{
			assertBlends[nAsserts] = &(b->second);
		}
		else if( chLabel.length() > NCHLAB-1 )
		{
	...
	else if( p.nMatch("DEPA") )
	{
		...
		if( !strAssertSpecies[nAsserts].empty() )
		{
			...
			strcpy( chAssertType[nAsserts] , "DC" );
		}
	...
	else if( p.nMatch("TEMP") )
	{
		...
		else if( (nelem = p.GetElem()) >= 0 )
		{
			 ...
			 * into array that will be used to get ionization after calculation */
			chAssertLineLabel[nAsserts] = elementnames.chElementNameShort[nelem];
			trimTrailingWhiteSpace( chAssertLineLabel[nAsserts] );
		}
		else
		{
			fprintf( ioQQQ, 
		...
		if( chAssertLineLabel[nAsserts] == "face" ||
		    chAssertLineLabel[nAsserts] == "21CM" ||
		    chAssertLineLabel[nAsserts] == "SPIN" ||
		    chAssertLineLabel[nAsserts] == "OPTI" )
		{
		...
		else if( chAssertLineLabel[nAsserts] == "H2" )
		{
			int j;
	...
	FILE * ioMONITOR )
{
	...
	for(i=0; i<nAsserts; ++i )
	{
		...
		if( strcmp( chAssertType[i] , "Lr")==0 )
		{
			...
			ipDisambiguate[i][0] = cdLine( chAssertLineLabel[i].c_str(), wavelength[i], &relint, &absint, iLineType[i] );
			if( ipDisambiguate[i][0] <= 0 )
			{
				fprintf( ioMONITOR, " monitor error: lgCheckMonitors could not find line ");
				prt_line_err( ioMONITOR, chAssertLineLabel[i].c_str(), wavelength[i] );
			...
			else
			{
				...
				strcpy(chFind, chAssertLineLabel[i].c_str());
				caps(chFind);

				/* get the error associated with specified significant figures */
				errorwave = WavlenErrorGet( wavelength[i], LineSave.sig_figs );
				...
				for( j=1; j < LineSave.nsum; j++ )
				{
					...
					if( fabs(LineSave.lines[j].wavelength()-wavelength[i]) < 3.f*errorwave )
					{
		...
		else if( strcmp( chAssertType[i], "Lb" ) == 0 )
		{
			if( cdLine( chAssertLineLabel[i].c_str(), wavelength[i], &relint, &absint, iLineType[i] ) <= 0 )
			{
				fprintf( ioMONITOR, " monitor error: lgCheckMonitors could not find line ");
				prt_line_err( ioMONITOR, chAssertLineLabel[i].c_str(), wavelength[i] );
			...
			double relint_cb = 0.,
				absint_cb = 0.;
			if( cdLine( "Ca B", wavelength[i], &relint_cb, &absint_cb, iLineType[i] ) <= 0 )
			{
				fprintf( ioMONITOR, " monitor error: lgCheckMonitors could not find line ");
				prt_line_err( ioMONITOR, chAssertLineLabel[i].c_str(), wavelength[i] );
		...
		{
//			long indice=cdLine( chAssertLineLabel[i].c_str(), wavelength[i], &relint, &absint );
//			fprintf(ioQQQ,"index line %ld %s %g \n", indice, LineSave.lines[indice].chALab(), LineSave.lines[indice].wavelength());
			...
			if (AssertError[i] > 0.0)
				...
				RelError[i] = PredQuan[i] - AssertQuantity[i];
			//	printf("i=%ld\t %s\t %g\t pred = %g\t asse = %g\t err = %g\n",
			//		i, chAssertLineLabel[i].c_str(), wavelength[i], PredQuan[i], AssertQuantity[i], RelError[i] );
		}
			...
			if( nISOCaseB == ipH_LIKE )
			{
				...
				for( long int ipLo=1+iCase; ipLo< MIN2(10,nHighestPrinted-1); ++ipLo )
				{
					for( long int ipHi=ipLo+1; ipHi< MIN2(25,nHighestPrinted); ++ipHi )
					{
						...
						cdLine( chAssertLineLabel[i].c_str() , wl , &CBrelint , &CBabsint, iLineType[i] );
						if( CBrelint < Param[i][4]  )
							continue;
						double error;
						/* now find the Case B intensity - may not all be present */
						if( cdLine( chElemLabelCaseB.c_str(), wl, &relint, &absint, iLineType[i] ) > 0 )
						{
							if (AssertError[i] > 0.)
		...
		else if( strcmp( chAssertType[i] , "DC") == 0 )
		{
			...
			const molezone *sp = getLevelsGeneric( this_species.c_str(), false, speciesLevels );
			if( sp == null_molezone )
			{
			...
			(strcmp( chAssertType[i] , "T ") == 0) )
		{
			...
			{
				long ion_stage = long( wavelength[i] );
				if( chAssertLineLabel[i] == "21CM" ||
				    chAssertLineLabel[i] == "SPIN" ||
				    chAssertLineLabel[i] == "OPTI" )
					ion_stage = 0;
					...
					chWeight )  )
				{
					fprintf( ioMONITOR, 
						" monitor error: lgCheckMonitors could not find an ion with label %s ion %li \n",
								chAssertLineLabel[i].c_str() , (long)wavelength[i] );
					/* go to next line */
					lg1OK[i] = false;
					RelError[i] = 0;
					PredQuan[i] = 0;
	...
	{ 
		...
		for( i=0; i<nAsserts; ++i )
		{
			PrtOneMonitor( ioMONITOR, chAssertType[i], chAssertLineLabel[i].c_str(), 
		...
		const double RelError, const double AssertError, const bool lg1OK, const bool lgQuantityLog, const bool lgFound )
{
	...
	if( strcmp( chAssertType, "Ll" )==0  || strcmp( chAssertType, "Lr" )==0 ||
		strcmp( chAssertType, "Lb" )==0 )
	{
