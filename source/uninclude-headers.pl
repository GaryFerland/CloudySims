...
#  for within #if-blocks cannot be moved to the beginning of the file.  More
...
my $compile_flags = '-DCLOUDY_DATA_PATH=\\\"some/path\\\"';
my $redundant_cpp = "redundant-headers-in-cpp.list";

my $tophead	  = "cddefines.h";
...
sub copy_all_but_include
{
	...
	foreach my $line ( @contents )
	{
		...
		next	if( $line =~ m/^#\s*include/ and $exclude =~ m/\b$header\b/ );
		print TMP $line;
	}
