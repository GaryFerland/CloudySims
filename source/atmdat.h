...
public:
   ...
   CollSplinesArray(void) : 
	collspline(NULL), SplineSecDer(NULL), EnergyDiff(0.), ScalingParam(0.) {}
	~CollSplinesArray()
   {
		if (collspline)
		{
...
public:
	...
	void setpoints(long ipHi, long ipLo, long ipCollider,long npoints)
	{
		...
		m_temps.resize(length);
		m_collstrs.resize(length);
 ...
 * The comment is a sequence of the lower and upper energy level indices
 * of the transition, as listed in the data file. */
inline string db_comment_tran_levels( long ipLoFile, long ipHiFile )
{
	stringstream Comment;
	Comment << ipLoFile << " " << ipHiFile;
	return Comment.str();
}
