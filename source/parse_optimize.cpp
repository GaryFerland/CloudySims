	...
	Parser &p)
{
	...
	if( p.nMatch("FILE") )
	{
		...
		if (p.GetQuote( str ))
			p.StringError();
		strcpy(chOptimFileName, str.c_str());
	}
...
STATIC void GetOptLineInt(Parser &p )
{
	...
	while( !p.m_lgEOF )
	{
		...
		optimize.chLineLabel.push_back( line.chLabel.c_str() );
		optimize.wavelength.push_back( line.wave );

		/* get the error associated with specified significant figures */
		optimize.errorwave.push_back( WavlenErrorGet(line.wave, LineSave.sig_figs ) );
...
STATIC void GetOptTemp(Parser &p )
{
	...
	while( !p.m_lgEOF )
	{
		...
		optimize.chTempLab.push_back( p.getFirstChunk(4) );
