		...
		for( long ion=0; ion < nelem-NISO+1; ion++ )
		{
			if(  dense.lgElmtOn[nelem] )
			{
				...
				else if( LineSave.ipass == 0 )
				{
					...
					// the constant 0.9998787 has been removed elsewhere in the code but has been
					// added here to prevent wavelengths from changing between c17.01 and later c17
					// versions; this could break existing scripts; see also ticket #407
					realnum wl = (realnum)(RYDLAM / (Heavy.Valence_IP_Ryd[nelem][ion]* 0.9998787));
					wl /= (realnum)RefIndex( 1e8/wl );
