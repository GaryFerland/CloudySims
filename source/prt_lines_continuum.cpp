...
void lines_continuum(void)
{
	...
	linadd( 0., t_vac(i), "####", 'i', " start continua");
	...
	if( geometry.iEmissPower == 2 )
	{
		...
		linadd(MAX2(0.,bac)/radius.dVeffAper,3646_air,"Bac ",'i',
		...
		linadd(f1/radius.dVeffAper,3645_air,"nFnu",'i',
		...
		linadd(f2/radius.dVeffAper,3647_air,"nFnu",'i',
		       "total flux above head of Balmer continuum, nuFnu ");
		/* >>chng 03 feb 06, set to zero */
		/* emslin saves the per unit vol emissivity of a line, which is normally 
		 * what goes into linadd.  We zero this unit emissivity which was set
		...
		linadd(MAX2(0.,bac)/radius.dVeffAper,3646_air,"cout",'i',
		...
		linadd(MAX2(0.,bac)/radius.dVeffAper,3646_air,"cref",'i',
		       "residual flux in Balmer continuum, nuFnu ");
		/* >>chng 03 feb 06, set to zero */
		/* emslin saves the per unit vol emissivity of a line, which is normally 
		 * what goes into linadd.  We zero this unit emissivity which was set
		...
		linadd(MAX2(0.,bac),3646_air,"thin",'i',
		       "residual flux in Balmer continuum, nuFnu ");

		linadd(continuum.cn4861/radius.dVeffAper,4860_air,"Inci",'i',
		       "incident continuum nu*f_nu at H-beta, at illuminated face of cloud ");

		linadd(continuum.cn1367/radius.dVeffAper,1367_vac,"Inci",'i',
		       "incident continuum nu*f_nu in FUV 1367A but out of Lya damping wings, at illuminated face of cloud");

		linadd(continuum.cn2066/radius.dVeffAper,2066_air,"Inci",'i',
		       "incident continuum nu*f_nu in FUV 2066A at illuminated face of cloud");

		linadd(continuum.cn1216/radius.dVeffAper,1215_vac,"Inci",'i',
	...
	linadd(MAX2(0.,CoolHeavy.brems_cool_net),0_vac,"HFFc",'c',
		"net free-free cooling, ALL species, free-free heating subtracted, so nearly cancels with cooling in LTE ");

	linadd(MAX2(0.,-CoolHeavy.brems_cool_net),0_vac,"HFFh",'h',
		"net free-free heating, nearly cancels with cooling in LTE ");

	linadd(CoolHeavy.brems_cool_h,0_vac,"H FF",'i',
		" H brems (free-free) cooling ");

	linadd(CoolHeavy.brems_heat_total,0_vac,"FF H",'i',
		"total free-free heating ");

	linadd(CoolHeavy.brems_cool_he,0_vac,"HeFF",'i',
		"He brems emission ");

	linadd(CoolHeavy.heavfb,0_vac,"MeFB",'c',
		"heavy element recombination cooling ");

	linadd(CoolHeavy.brems_cool_metals,0_vac,"MeFF",'i',
		"heavy elements (metals) brems cooling, heat not subtracted ");

	linadd(CoolHeavy.brems_cool_h+CoolHeavy.brems_cool_he+CoolHeavy.brems_cool_metals,0_vac,"ToFF",'i',
		"total brems emission - total cooling but not minus heating ");

	linadd((CoolHeavy.brems_cool_h+CoolHeavy.brems_cool_he)*sexp(5.8e6/phycon.te),0_vac,"FF X",'i',
		"part of H brems, in x-ray beyond 0.5KeV ");

	linadd(CoolHeavy.eebrm,0_vac,"eeff",'c',
		"electron - electron brems ");

	linadd(CoolHeavy.colmet,0_vac,"Mion",'c',
	...
	i = StuffComment( "RRC" );
	linadd( 0., t_vac(i), "####", 'i', "radiative recombination continua");
		...
		for( long nelem=ipISO; nelem < LIMELM; nelem++ )
		{
			if( nelem < 2 || dense.lgElmtOn[nelem] )
			{
				for( long n=0; n < iso_sp[ipISO][nelem].numLevels_max; n++ )
				{
					...
					{
						// save intensity
						linadd(iso_sp[ipISO][nelem].fb[n].RadRecCon,0_vac,"dumy",'i',
							"radiative recombination continuum");
					}
				}
			}
	...
	for( long nelem=NISO; nelem < LIMELM; nelem++ )
	{
		...
		for( long ion=0; ion < nelem-NISO+1; ion++ )
		{
			if(  dense.lgElmtOn[nelem] )
			{
				...
				{
					// save intensity
					linadd(Heavy.RadRecCon[nelem][ion],0_vac,"dumy",'i',
						"radiative recombination continuum");
				}
			}
		}
