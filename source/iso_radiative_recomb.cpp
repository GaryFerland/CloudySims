						  ...
						  long nelem )
{
	...
	if( !fp_equal(phycon.te,TeUsed[ipISO][nelem]) || iso_sp[ipISO][nelem].lgMustReeval || !conv.nTotalIoniz || cosmology.lgDo )
	{
		...
		for( ipLevel=ipFirstCollapsed; ipLevel<iso_sp[ipISO][nelem].numLevels_local; ++ipLevel )
		{
			...
			if (N_(ipLevel)< NHYDRO_MAX_LEVEL)
				RadRec = t_ADfA::Inst().H_rad_rec(nelem+1-ipISO, N_(ipLevel), phycon.te);
			else
				RadRec = Recomb_Seaton59( nelem+1-ipISO, phycon.te, N_(ipLevel) );
		...
		else
		{
			...
			if( iso_ctrl.lgNoRecombInterp[ipISO] )
			{
				...
				/* Max collapsed n and max level to include in the rad. recombination */
				long N_MaxCollapsed = N_(ipFirstCollapsed) + iso_sp[ipISO][nelem].nCollapsed_max;
				long N_MaxRadRec = MAX2(N_MaxCollapsed,SumUpToThisN);

				for( long nLo = N_MaxCollapsed; nLo < NHYDRO_MAX_LEVEL; nLo++ )
				{
					TotalRadRecomb += t_ADfA::Inst().H_rad_rec(nelem+1-ipISO, nLo, phycon.te);
				}
				/* Plus a bunch more for good measure */
				for( long nLo = NHYDRO_MAX_LEVEL; nLo<=N_MaxRadRec; nLo++ )
				{
				...
				for( nelem = ipISO; nelem < LIMELM; nelem++ )
				{
					if( dense.lgElmtOn[nelem] )
					{
						...
						* interpolate for a given temperature, just calculate it directly with a hydrogenic routine.	*/
						...
						{
							/* Max collapsed n and max level to include in the rad. recombination */
							long N_MaxCollapsed =  iso_sp[ipISO][nelem].n_HighestResolved_max+1+ iso_sp[ipISO][nelem].nCollapsed_max;
							long N_MaxRadRec = MAX2(N_MaxCollapsed,SumUpToThisN);
							...
							for( i1 = NHYDRO_MAX_LEVEL; i1<=N_MaxRadRec; i1++ )
							{
