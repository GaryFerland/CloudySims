...
void get_pivotal( long nelem, long ipISO, long *mmax, long m[][NISO]);
	...
	for( long ipISO=ipH_LIKE; ipISO<NISO; ++ipISO )
	{
		...
		for( long nelem=ipISO; nelem < LIMELM; nelem++ )
		{
			if( dense.lgElmtOn[nelem] )
			{
				...
				long m[iso_sp[ipISO][nelem].numLevels_max][NISO];

				/* Only do this if condensation is allowed and the max n is over the minimum number of condensed levels */
				if (!iso_ctrl.lgMatCondOff[ipISO][nelem] && dense.lgElmtOn[nelem])
				{
					get_pivotal(nelem, ipISO, &iso_sp[ipISO][nelem].mmax,m);

					for (ipHi=0; ipHi < iso_sp[ipISO][nelem].mmax; ipHi++)
						iso_sp[ipISO][nelem].fb[ipHi].ipm = m[ipHi][ipISO];
				}
...
{
/*
 * Pivotal levels are automatically obtained here. Pivotal levels should work for l-unresolved levels.
 * The following rules will be followed for this calculation
 *
 * 1) The minimum number of condensed levels will be the default number
 * of resolved levels plus five collapsed levels for each iso-sequence for each ion
 *
 * 2) All resolved levels are condensed.
 *
 * 3) The n gap will be incrementing every 5 steps by 1.
 *
 */
	...
	long ipncrit = sp->numLevels_local - sp->nCollapsed_local;
	for (long ipLo=0; ipLo < sp->numLevels_local - sp->nCollapsed_local; ipLo++)
	{
		/* Ground level and metastables are always included
		 * N=1 and N=2 are included */
		m[ipLo][ipISO] = ipLo;
	}
	...
	ASSERT( N_(ipncrit) >= sp->n_HighestResolved_local);


	long ncol = N_(m[t][ipISO]) + 1;
	long gap = 1;
	long step = 0;
	long step0 = 0;
	if (t==ipncrit)
	...
	while (ncol < sp->nCollapsed_local +sp->n_HighestResolved_local)
	{
		/* Uncomment to reduce the gap at the high limit */
		/*if(ncol > sp->nCollapsed_local +sp->n_HighestResolved_local -30)
		gap = 4;
		if(ncol > sp->nCollapsed_local +sp->n_HighestResolved_local -20)
		gap = 3;
		if(ncol > sp->nCollapsed_local +sp->n_HighestResolved_local -10)
		gap = 2;
		if(ncol > sp->nCollapsed_local +sp->n_HighestResolved_local -5)
		gap = 1;
		*/

		m[t][ipISO] = m[t-1][ipISO]+gap;
		t++;

		ncol+=gap;
		step++;
		step0++;
		if (step0 >= 10 && step%5 ==0 )
			gap++;
	}


	if(ncol > sp->nCollapsed_local +sp->n_HighestResolved_local)
		t--;

	/* Including the last level */
	m[t][ipISO] = sp->numLevels_local - 1;
	...
	*mmax = t+1;
	...
	fprintf(ioQQQ,"highest resolved n %li, number of collapsed %li\n",
			sp->n_HighestResolved_local,sp->nCollapsed_local);

	for (long j=0; j<t+1; j++)
		fprintf(ioQQQ,"m[%li][%li] = %li\n",j,ipISO,m[j][ipISO]);
	//*/
