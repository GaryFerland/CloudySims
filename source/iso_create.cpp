			...
			if( nelem < 2 || dense.lgElmtOn[nelem] )
			{
				...
				for( ipHi=0; ipHi < iso_sp[ipISO][nelem].numLevels_max; ipHi++ )
				{
					...
					if( ipISO == ipH_LIKE )
					{
						...
						if( ipHi==ipH2p && nelem >4 )
						{
							/* only Ka for Z>5 */
							double Z = nelem+1;
							/* >>20 sept 24, Priyanka Chakraborty's energy correction in wavenumbers
							*updated phfit.dat according to NIST values for H-like ions
							*Compared Ritz formula with NIST and 
							*generated correction to the polynomial */
							double E_correction = 0.1783*pow(Z,4)-1.8313*pow(Z,3)+27.803*pow(Z,2)-208.04*Z+570.59;
							/* convert to Ryd and apply */
							EnergyRyd -= E_correction * WAVNRYD;
						}
					}
		...
		for( long nelem=ipISO; nelem < LIMELM; ++nelem )
		{
			...
			if( nelem < 2 || dense.lgElmtOn[nelem] )
			{
...

				{
					...
					if( strcmp( chemicalLabel.c_str(), prt.matrix.species ) == 0 )
						sp->lgPrtMatrix = true;
				}
				...
				{
					// NB - this is the total number of levels we would have if all n were resolved.  
					long numLevels = iso_get_total_num_levels( ipISO, sp->n_HighestResolved_max + sp->nCollapsed_max, 0 );
					sp->CachedAs.reserve( i, numLevels );
					for( long i1 = 0; i1 < numLevels; ++i1 )
					{
				...
				sp->IndexIfAllResolved.alloc( sp->QuantumNumbers2Index.clone() );
				sp->QuantumNumbers2Index.invalidate();
				sp->IndexIfAllResolved.invalidate();
			}
		}
	}
		...
		if( nelem == ipHELIUM || dense.lgElmtOn[nelem] )
		{
			...
			for( in = 1L; in <= iso_sp[ipISO][nelem].n_HighestResolved_max; ++in )
			{
				for( il = 0L; il < in; ++il )
				{
					...
					iso_sp[ipISO][nelem].IndexIfAllResolved[in][il][is] = i;
					++i;
				}
			}
			/* Now find indices of levels if all were resolved */
			{
				long i2 = i;
				for( in = iso_sp[ipISO][nelem].n_HighestResolved_max + 1; 
					in <= iso_sp[ipISO][nelem].n_HighestResolved_max + iso_sp[ipISO][nelem].nCollapsed_max; ++in )
				{
					for( il = 0L; il < in; ++il )
					{
						iso_sp[ipISO][nelem].IndexIfAllResolved[in][il][is] = i2;
						++i2;
					}
				}
			}
			...
			for( in = 2L; in <= iso_sp[ipISO][nelem].n_HighestResolved_max + iso_sp[ipISO][nelem].nCollapsed_max; ++in )
			{
				for( il = 0L; il < in; ++il )
				{
					...
					if( in <= iso_sp[ipISO][nelem].n_HighestResolved_max )
					{
						...
						// QuantumNumbers2Index and IndexIfAllResolved must agree for resolved levels
						ASSERT( iso_sp[ipISO][nelem].QuantumNumbers2Index[in][il][is] == iso_sp[ipISO][nelem].IndexIfAllResolved[in][il][is] );
					}
				}
			}
		}
		...
		if( nelem == ipHELIUM || dense.lgElmtOn[nelem] )
		{
			...
			for( in = 1L; in <= iso_sp[ipISO][nelem].n_HighestResolved_max; ++in )
			{
				...
				if( in > 1L )
				{
					...
					iso_sp[ipISO][nelem].IndexIfAllResolved[in][1][1] = i;
					++i;
				}
			}
			/* Now find indices of levels if all were resolved */
			{
					...
					in <= iso_sp[ipISO][nelem].n_HighestResolved_max + iso_sp[ipISO][nelem].nCollapsed_max; ++in )
				{
					...
					if( in > 1L )
					{
						iso_sp[ipISO][nelem].IndexIfAllResolved[in][1][1] = i2;
						++i2;
					}
				}
			}
...
void iso_collapsed_Aul_update( long ipISO, long nelem )
{
	...
	for( long ipLo=0; ipLo<ipFirstCollapsed; ipLo++ )
	{
		...
		{
			// NB - 2nd dim of CachedAs is indexed properly by IndexIfAllResolved, but ipLo is always resolved here, so there is no difference.
			realnum Auls[2] = {
			...
			realnum EffectiveAul = 
				Auls[0]*spin*(2.f*(L_(ipLo)+1.f)+1.f);
			...
			{
				EffectiveAul += 
					Auls[1]*spin*(2.f*(L_(ipLo)-1.f)+1.f);
			}
...
{
	DEBUG_ENTRY( "iso_collapsed_lifetimes_update()" );
	...
	for( long ipHi=iso_sp[ipISO][nelem].numLevels_max- iso_sp[ipISO][nelem].nCollapsed_max; ipHi < iso_sp[ipISO][nelem].numLevels_max; ipHi++ )
	{
		...
		for( long ipLo=0; ipLo < ipHi; ipLo++ )  
		{
			...
			totalAul += iso_sp[ipISO][nelem].trans(ipHi,ipLo).Emis().Aul();
		}

		/* sum of A's was just stuffed, now invert for lifetime. */
		iso_sp[ipISO][nelem].st[ipHi].lifetime() = 1./totalAul;
		...
		for( long ipLo=0; ipLo < ipHi; ipLo++ )
		{
			...
			iso_sp[ipISO][nelem].trans(ipHi,ipLo).Emis().dampXvel() = (realnum)( 
				totalAul/(PI4*iso_sp[ipISO][nelem].trans(ipHi,ipLo).EnergyWN()));
