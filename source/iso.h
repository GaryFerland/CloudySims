...
/**iso_departure_solver solve for iso-sequence ionization balance
...
*/
void iso_departure_solver( const long ipISO, const long nelem, double& renorm,
		bool lgPrtMatrix );
...
public:
	...
	/** switches ON and OFF the matrix condensation	 */
	bool lgMatCondOff[NISO][LIMELM];

	/** Calculates only once the extra term for the condensation matrix */
	bool lgNoExtraTermCalc[NISO][LIMELM];
