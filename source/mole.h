...
{
public:
	bool operator()(const count_ptr<chem_nuclide>& a,
						const count_ptr<chem_nuclide>& b) const
	{
		return *a < *b;
	}
};
...
class molecule {
public:
	typedef map<const count_ptr<chem_nuclide>, int,
...
extern count_ptr<chem_nuclide> findnuclide(const char buf[]);
...
public:
	...
	const molecule* global() const
	{
		if (index == -1)
			return null_mole;
		return mole_global.list[index].get_ptr();
	}
