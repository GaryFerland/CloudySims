...
STATIC void mole_generate_isotopologue_reactions( string atom_old, string atom_new )
{
	...
	count_ptr<chem_nuclide> atomOld = findnuclide(atom_old.c_str());
...
STATIC void parse_udfa(char *s)
{
	...
	count_ptr<udfa_reaction>r(new udfa_reaction);
	f = getcsvfield(&s,',');
	r->index = atoi(f);
...
STATIC void compare_udfa(const count_ptr<mole_reaction> &rate)
{
	...
	map<formula_species,	count_ptr<udfa_reaction> >::iterator p
		= udfatab.find(s);
	if (p == udfatab.end() )
	{
	...
	{
		count_ptr<udfa_reaction>& u = p->second;
...
STATIC void mole_h_reactions( void )
{
	...
	/*hmi.HMinus_photo_rate = GammaBn( hmi.iphmin, nhe1Com.nhe1[0] , opac.iphmop ,
	  HMINUSIONPOT , &hmi.HMinus_induc_rec_rate , &hmi.HMinus_induc_rec_cooling );*/
	hmi.HMinus_photo_rate = GammaBn( hmi.iphmin, iso_sp[ipHE_LIKE][ipHELIUM].fb[0].ipIsoLevNIonCon , opac.iphmop ,
												HMINUSIONPOT , &hmi.HMinus_induc_rec_rate , &hmi.HMinus_induc_rec_cooling, &photoHeat );
...
double t_mole_local::dissoc_rate(const char chSpecies[]) const
{
	...
	const chem_nuclide *tgt = sp->nNuclide.begin()->first.get_ptr();
				 ...
				 =mole_priv::reactab.begin(); p != mole_priv::reactab.end(); ++p) 
	{
		...
		for (int i=0;i<rate.nreactants;i++)
		{
			if (rate.reactants[i] == sp)
				++ipspin;
			if (rate.reactants[i]->isMonatomic() && tgt == sp->nNuclide.begin()->first.get_ptr()) 
				++ipfreein;
		}
		...
		for (int i=0;i<rate.nproducts;i++)
		{
			if (rate.products[i] == sp) 
				++ipspout;
			if (rate.products[i]->isMonatomic() && tgt == sp->nNuclide.begin()->first.get_ptr()) 
				++ipfreeout;
		}
