...
void ParseCommands(void)
{
	...
	sprintf(chDUMMY, "abundances \"default.abn\"" );
	p.setline(chDUMMY);
	ParseAbundances( p );

	// set default isotopic abundances
	sprintf( chDUMMY, "abundances isotopes \"default-iso.abn\"" );
	p.setline( chDUMMY );
	ParseAbundances( p );
	...
	if( input.lgDeprecatedComment ) {
		fprintf( ioQQQ, "\n NOTE: one of these comment characters was found in the input: //, %%, *, or ;.\n" );
		fprintf( ioQQQ, " Using these is deprecated and they will be removed in the next major release.\n" );
		fprintf( ioQQQ, " Please convert your input deck using scripts/ccc.pl.\n\n" );
	}
...
void ParseIterations(Parser &p)
{
	...
	if( iterations.itermx > iterations.iter_malloc - 1 )
	{
		...
		iterations.IterPrnt.resize((size_t)iterations.iter_malloc);
		iterations.nend.resize((size_t)iterations.iter_malloc);
		iterations.StopThickness.resize((size_t)iterations.iter_malloc);
		iterations.StopRadius.resize((size_t)iterations.iter_malloc);
		...
		for(long int j=iter_malloc_save; j<iterations.iter_malloc; ++j )
		{
			...
			iterations.nend[j] = iterations.nend[iter_malloc_save-1];
			iterations.StopThickness[j] = iterations.StopThickness[iter_malloc_save-1];
			iterations.StopRadius[j] = iterations.StopRadius[iter_malloc_save-1];
		}
	}
...
void ParseTitle(Parser &p)
{
	/* read in title of model starting in col 5 -- prefer to get string
		...
		can get */
	string title;
	if (p.GetQuote(title) != 0)
		strcpy( input.chTitle , p.getRawTail().c_str() );
	else
		strcpy( input.chTitle , title.c_str() );
}	
void ParseTolerance(Parser &)
{
