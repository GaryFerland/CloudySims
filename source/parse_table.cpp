...
void ParseTable(Parser &p)
{
	...
	if( p.nMatch(" AGN") )
	{
		...
		for( i=0; i < NAGN; i++ )
		{
			rfield.tNu[rfield.nShape].push_back( tnuagn[i] );
			rfield.tslop[rfield.nShape].push_back( tslagn[i] );
		}
	...
	else if( p.nMatchErase("HM96") )
	{
		...
		* >>refer	continuum	background	Haardt, Francesco, & Madau, Piero, 1996, ApJ, 461, 20 */
		for( j=0; j < NHM96; j++ )
		{
			...
			rfield.tNu[rfield.nShape].push_back( Energy(exp10( tnuHM96[j])) );
			rfield.tslop[rfield.nShape].push_back( (realnum)fnuHM96[j] );
		}
	...
	else if( p.nMatch(" ISM") )
	{
		 ...
		 * any redshift */
		rfield.tNu[rfield.nShape].push_back( Energy( exp10(6.)/FR1RYD) );
		rfield.tslop[rfield.nShape].push_back( (-21.21f - 6.f) );
		for( i=6; i < NISM; i++ )
		{
			/* energies were stored as log Hz and intensity as log nu Fnu, as per John's plot.
			 * convert to Ryd and log Fnu */
			rfield.tNu[rfield.nShape].push_back( Energy( exp10(tnuism[i])/FR1RYD ) );
			rfield.tslop[rfield.nShape].push_back( (realnum)(fnuism[i] - tnuism[i]) );
		}
	...
	else if( p.nMatch("DRAI") )
	{
		...
		for( i=0; i < NDRAINE; i++ )
		{
			rfield.tNu[rfield.nShape].push_back( Energy(tnudrn[i]) );
			rfield.tslop[rfield.nShape].push_back( (realnum)tsldrn[i] );
		}
	...
	else if( p.nMatch("POWE") )
	{
		...
		rfield.tNu[rfield.nShape].push_back( Energy(rfield.emm()) );
		/* and the value of the flux at this point (f_nu units)*/
		rfield.tslop[rfield.nShape].push_back( -5.f );
		...
		rfield.tNu[rfield.nShape].push_back( Energy(brakmm) );
		...
		rfield.tNu[rfield.nShape].push_back( Energy(brakxr) );

		/* this is energy of the high-energy limit to code */
		rfield.tNu[rfield.nShape].push_back( Energy(rfield.egamry()) );
	...
	else if( p.nMatch("SED") || lgKeyword )
	{
		...
		/* now open the data file -- first try local directory, then data/SED */
		...
		/* read data first time, counting size of required tables,
		 * and parse user optional keywords */
		long int fileLength = 0;
		const char *chEnergyUnits=NULL;
		bool lgUnitsSet = false, lgNuFnu = false, lgExtrapolate = false;

		while( read_whole_line( chLine , (int)sizeof(chLine) , ioDATA ) != NULL )
		{
			// field of stars ends data
			if( chLine[0]=='*' )
				break;
			...
			if( chLine[0]=='\n' || chLine[0]=='\0' )
			{
				fprintf(ioQQQ, "PROBLEM in TABLE SED: Encountered unexpected empty line.\n");
				cdEXIT(EXIT_FAILURE);
			}

			/* copy input to CAPS to parse keywords */
			char chLineCAPS[INPUT_LINE_LENGTH];
			strcpy( chLineCAPS , chLine );
			caps( chLineCAPS );

			/* change photon units */
			if(nMatch( "UNIT", chLineCAPS ) != 0)
			{
				chEnergyUnits = StandardEnergyUnit(chLineCAPS);
				lgUnitsSet = true;
			}

			/* default is F_nu - this makes it nu F_nu */
			if(nMatch( "NUFN" , chLineCAPS ) != 0)
				lgNuFnu = true;

			/* default is use exactly the specified continuum, extrapolate option
			 * will extrapolate lowest energy point to low-energy limit of the code */
			if(nMatch( "EXTRAP" , chLineCAPS ) != 0)
				lgExtrapolate = true;
		...
		vector<double> tnuInput(fileLength);
		vector<double> tslopInput(fileLength);
		rewind(ioDATA);
		...
		while( read_whole_line( chLine , (int)sizeof(chLine) , ioDATA ) != NULL )
		{
			...
			long i = 1;
			tnuInput[entryNum] = FFmtRead(chLine,&i,sizeof(chLine),&lgEOL);
			if( lgEOL)
			{
				fprintf(ioQQQ,"PROBLEM in TABLE SED: freq/wavl and flux pairs must be entered."
					"  The first number was not found on this line:\n%s\n", chLine);
				cdEXIT(EXIT_FAILURE);
			}
			tslopInput[entryNum] = FFmtRead(chLine,&i,sizeof(chLine),&lgEOL);
			/* these were not checked during the first read through the data since
			 * no data were actually parsed
			 */
			if( lgEOL)
			{
				fprintf(ioQQQ,"PROBLEM in TABLE SED: freq/wavl and flux pairs must be entered."
					"  The second number was not found on this line:\n%s\n", chLine);
				cdEXIT(EXIT_FAILURE);
			}

			if( tslopInput[entryNum] <= 0. )
			{
				fprintf(ioQQQ,"PROBLEM flux in TABLE SED file entry %li is not positive\nphoton energy %e where the value is %e.\n",
					entryNum , tnuInput[entryNum] , tslopInput[entryNum] );
				fprintf(ioQQQ,"Line image follows:\n%s\n", chLine );
				cdEXIT(EXIT_FAILURE);
			}

			if( tnuInput[entryNum] <= 0. )
			{
				fprintf(ioQQQ,"PROBLEM freq/wavl in TABLE SED file entry %li is not positive\n"
					"freq/wavl is %e.\n",
					entryNum , tnuInput[entryNum] );
				fprintf(ioQQQ,"Line image follows:\n%s\n", chLine );
				cdEXIT(EXIT_FAILURE);
			}
			...
			{
				fprintf(ioQQQ,"PROBLEM freq/wavl values in TABLE SED file must be in increasing or decreasing order\n");
				fprintf(ioQQQ, "The following freq/wavl sequence is not monotonically changing:\n");
				for( long ij=entryNum-2; ij<=entryNum; ++ij )
					fprintf(ioQQQ, "%li %e\n", ij, tnuInput[ij] );
				cdEXIT(EXIT_FAILURE);
			}

			tslopInput[entryNum] = log10(tslopInput[entryNum]);
			entryNum++;
		}

		ASSERT( entryNum == fileLength );
		...
		/* we want to specify log F_nu - this converts linear nh F+nu to log F_nu */
		if( lgNuFnu )
		{
			for( long i=0; i < entryNum; i++ )
				tslopInput[i] -= log10(tnuInput[i]);
		}
		...
		if( tnuInput[0] < tnuInput[1] )
		{
			for( long i=0; i < entryNum; i++ )
			{
				rfield.tNu[rfield.nShape].push_back( Energy(tnuInput[i]) );
				rfield.tslop[rfield.nShape].push_back( (realnum)tslopInput[i] );
			}
		}
		else
		{
			for( long i=0; i < entryNum; i++ )
			{
				rfield.tNu[rfield.nShape].push_back( Energy(tnuInput[entryNum-i-1]) );
				rfield.tslop[rfield.nShape].push_back( (realnum)tslopInput[entryNum-i-1] );
			}
		}
		...
		if( lgExtrapolate )
			resetBltin( get_ptr(rfield.tNu[rfield.nShape]), get_ptr(rfield.tslop[rfield.nShape]), true );
	...
	{
		char chMetalicity[5] = {'\0'}, chODFNew[8], chVaryFlag[7] = {'\0'};
		...
		for (i=0; i < (long)(sizeof(table)/sizeof(*table)); ++i)
		{
			...
			{
				strncpy( chVaryFlag, table[i][0], sizeof(chVaryFlag)-1 );
				strncpy( chMetalicity, table[i][1], sizeof(chMetalicity)-1 );
				break;
			}
		}
		...
		if( lgQuoteFound )
		{
			nstar = GridInterpolate(val,&nval,&ndim,chFile.c_str(),lgList,&Tlow,&Thigh);
		}
	...
	else
	{
		fprintf( ioQQQ, " There MUST be a keyword on this line. The keys are:"
			"AGN, DRAINE, HM96, HM05, HM12, _ISM, LINES, POWERlaw, "
			"READ, SED, and STAR.\n  Sorry.\n" );
		cdEXIT(EXIT_FAILURE);
	}
	...
	for( i=0; i < rfield.ncont[rfield.nShape]; i++ )
	{
		rfield.tFluxLog[rfield.nShape].push_back( rfield.tslop[rfield.nShape][i] );
	}
...
int lines_table()
{
	...
	{
		if( LineSave.findline( chLabel[n].c_str(), wl[n] ) <= 0 )
		{
			++miss;
			fprintf(ioQQQ,"lines_table in parse_table.cpp did not find line %s ",chLabel[n].c_str());
			prt_wl(ioQQQ,wl[n]);
			fprintf(ioQQQ,"\n");
		}
	}
...
STATIC void ReadTable(const char *fnam)
{
	...
	if( strncmp( md5sum, rfield.mesh_md5sum().c_str(), NMD5 ) != 0 ||
	...
	if( read_whole_line( chLine , (int)sizeof(chLine) , io )!=NULL )
	{
		sscanf( chLine, "%le", &rfield.TableRadius[rfield.nShape] );
	}
	else
	{
		fprintf( ioQQQ, " the input continuum file has been truncated.\n" );
	...
	while( read_whole_line(chLine, (int)sizeof(chLine),io) != NULL )
	{
		...
		rfield.tNu[rfield.nShape].push_back( Energy(help[0]) );
		// Convert to standard FluxLog units
		if (help[1] > 0.0)
		{
			rfield.tFluxLog[rfield.nShape].push_back( (realnum)log10(help[1]/help[0]) );
		}
		else
		{
			rfield.tFluxLog[rfield.nShape].push_back( realnum(-70.) );
		}
		++i;
	}
