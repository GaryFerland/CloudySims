		     ...
		     double e)
{
	...
	{
		/* 07/06/2017 -- The fits have been extrapolated to higher n using a power law where the index
		 * depends slightly on the photon energy. Doing that we get a smooth transition
		 * even at high energies */

		double k=7.797e-18;
        double index=-3.0784-0.010531*log(e);
        cs = (k*n+9.2265e-19)*pow(ex,index)/POW2((double)iz);
        ASSERT(cs >= 0.);
	}
	return cs;
}
