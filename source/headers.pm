...
sub get_thread_lims
{
	...
	{
		my $files_per_thread = int( @files / ($ncpus-1) ) + (@files % ($ncpus-1) > $ncpus/2 ? 1 : 0);
		#	print $files_per_thread ."\n";
		for( my $i = 0; $i < $ncpus; $i++ )
		{
			my $s = $i * $files_per_thread;
			last	if( $s >= @files );
			my $e = $s + $files_per_thread - 1;
			   $e = @files - 1
				if( $e >= @files );
			$ibeg[$i] = $s;
			$iend[$i] = $e;
...
sub do_compile
{
	...
	my $out = `g++ -c -Wall -Wmissing-declarations $cpp $compile_flags -o $obj 2>&1`;
	#	print "$cpp:\t$out\n"	if( $out ne "" and $out ne "\n" );
