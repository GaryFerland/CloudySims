...
void mole_make_list()
{
	...
	for(i=0;i<mole_global.num_total;i++) 
	{
		...
		if (sp->isMonatomic())
		{
			...
			count_ptr<chem_nuclide> atom = sp->nNuclide.begin()->first;
	...
	realnum mass_amu, double frac )
{
	...
	count_ptr<chem_nuclide> isotope(new chem_nuclide);
	...
	bool lgCreateIsotopologues )/* formation enthalpy at 0K */
{
	...
	if(type == MOLECULE)
	{
		if( parse_species_label( mylab, nuclidesLeftToRight, numNuclides, embellishments, mol->lgExcit, mol->charge, mol->lgGas_Phase ) == false )
			return NULL;		
	...
	if( lgCreateIsotopologues && type==MOLECULE && !mol->isMonatomic() )
	{
		...
		{
			for( map<int, count_ptr<chem_nuclide> >::iterator it2 = it1->first->el->isotopes.begin(); 
...
count_ptr<chem_nuclide> findnuclide(const char buf[])
{
	...
	if(p != nuclidetab.end())  
		...
		return count_ptr<chem_nuclide>(NULL);
}
