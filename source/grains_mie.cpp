...
template<typename T>
STATIC void mie_read_number(/*@in@*/const string&,/*@in@*/const string&,/*@out@*/T*,bool,long int);
				  ...
				  /*@in@*/const GrainPar& gp)
{
	...
	mie_read_number(chFile,chLine,&magic,true,dl);
	if( magic != MAGIC_OPC ) 
	{
	...
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gv.bin[nd].dustp[0],true,dl);
	...
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gv.bin[nd].dustp[1],true,dl);

	/* average molecular weight per atom (amu) */
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gv.bin[nd].atomWeight,true,dl);

	/* abundance of grain molecule for max depletion */
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gv.bin[nd].dustp[2],true,dl);

	/* depletion efficiency */
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gv.bin[nd].dustp[3],true,dl);
	...
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gv.bin[nd].AvRadius,true,dl);
	gv.bin[nd].eyc = 1./gv.bin[nd].AvRadius + 1.e7;

	/* average grain area <4pi*a^2> for entire size distr (cm^2) */
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gv.bin[nd].AvArea,true,dl);

	/* average grain volume <4/3pi*a^3> for entire size distr (cm^3) */
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gv.bin[nd].AvVol,true,dl);

	/* total grain radius Int(a) per H for entire size distr (cm/H) */
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gv.bin[nd].IntRadius,true,dl);

	/* total grain area Int(4pi*a^2) per H for entire size distr (cm^2/H) */
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gv.bin[nd].IntArea,true,dl);

	/* total grain vol Int(4/3pi*a^3) per H for entire size distr (cm^3/H) */
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gv.bin[nd].IntVol,true,dl);

	/* work function, in Rydberg */
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gv.bin[nd].DustWorkFcn,true,dl);

	/* bandgap, in Rydberg */
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gv.bin[nd].BandGap,false,dl);

	/* efficiency of thermionic emissions, between 0 and 1 */
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gv.bin[nd].ThermEff,true,dl);

	/* sublimation temperature in K */
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gv.bin[nd].Tsublimat,true,dl);

	/* material type, determines enthalpy function, etc... */
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&help,true,dl);
	gv.bin[nd].matType = (mat_type)help;

	for( nelem=0; nelem < LIMELM; nelem++ ) 
	{
		mie_next_data(chFile,io2,chLine,&dl);
		mie_read_number(chFile,chLine,&RefAbund[nelem],false,dl);
	...
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&RadiusRatio,true,dl);
	...
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&nup,false,dl);
	for( i=0; i < nup; i++ )
		mie_next_data(chFile,io2,chLine,&dl);
	...
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&nup,true,dl);

	/* no. of size distribution bins */
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&nbin,true,dl);
	...
	if( nbin > 1 )
	{
		...
		for( i=0; i < nbin; i++ )
		{
			...
			mie_next_data(chFile,io2,chLine,&dl);
			mie_read_number(chFile,chLine,&gv.bin[nd2].AvRadius,true,dl);

			/* average grain area in this bin (cm^2) */
			mie_next_data(chFile,io2,chLine,&dl);
			mie_read_number(chFile,chLine,&gv.bin[nd2].AvArea,true,dl);

			/* average grain volume in this bin (cm^3) */
			mie_next_data(chFile,io2,chLine,&dl);
			mie_read_number(chFile,chLine,&gv.bin[nd2].AvVol,true,dl);

			/* total grain radius Int(a) per H for this bin (cm/H) */
			mie_next_data(chFile,io2,chLine,&dl);
			mie_read_number(chFile,chLine,&gv.bin[nd2].IntRadius,true,dl);

			/* total grain area Int(4pi*a^2) per H for this bin (cm^2/H) */
			mie_next_data(chFile,io2,chLine,&dl);
			mie_read_number(chFile,chLine,&gv.bin[nd2].IntArea,true,dl);

			/* total grain vol Int(4/3pi*a^3) per H for this bin (cm^3/H) */
			mie_next_data(chFile,io2,chLine,&dl);
			mie_read_number(chFile,chLine,&gv.bin[nd2].IntVol,true,dl);
						 ...
						 /*@out@*/ grain_data *gd)
{
	...
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gd->magic,true,dl);
	...
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gd->abun,true,dl);

	/* default depletion of grain molecule */
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gd->depl,true,dl);
	if( gd->depl > 1. ) 
	{
	...
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gd->rho,false,dl);
	...
	/* work function, in Rydberg */
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gd->work,true,dl);

	/* bandgap, in Rydberg */
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gd->bandgap,false,dl);
	...
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gd->therm_eff,true,dl);
	if( gd->therm_eff > 1.f ) 
	{
	...
	/* sublimation temperature in K */
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&gd->subl_temp,true,dl);
	...
	case RFI_TABLE:
		...
		mie_next_data(chFile,io2,chLine,&dl);
		mie_read_number(chFile,chLine,&nridf,true,dl);
		if( nridf > 3 ) 
		{
		...
		mie_next_data(chFile,io2,chLine,&dl);
		mie_read_number(chFile,chLine,&gd->nAxes,true,dl);
		if( gd->nAxes > NAX ) 
		{
		...
		{
		case 1:
			mie_read_number(chFile,chLine,&gd->wt[0],true,dl);
			total = gd->wt[0];
			break;
		case 2:
		...
		for( j=0; j < gd->nAxes; j++ ) 
		{
			...
			mie_next_data(chFile,io2,chLine,&dl);
			mie_read_number(chFile,chLine,&gd->ndata[j],false,dl);
			if( gd->ndata[j] < 2 ) 
			{
	...
	case OPC_TABLE:
		...
		mie_next_data(chFile,io2,chLine,&dl);
		mie_read_number(chFile,chLine,&gd->nOpcCols,true,dl);
		if( gd->nOpcCols > NDAT ) 
		{
		...
		mie_next_data(chFile,io2,chLine,&dl);
		mie_read_number(chFile,chLine,&gd->nOpcData,false,dl);
		if( gd->nOpcData < 2 ) 
		{
			fprintf( ioQQQ, " Illegal number of data points in %s\n",chFile.c_str() );
			fprintf( ioQQQ, " Line #%ld, number=%ld\n",dl,gd->nOpcData);
						 ...
						 /*@out@*/ grain_data *gd)
{
	...
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&nMaterial,true,dl);
	if( nMaterial < 2 ) 
	{
	...
	for( i=0; i < nMaterial; i++ )
	{
		...
		mie_next_data(chFile,io2,chLine,&dl);
		mie_read_number(chFile,chLine,&frac[i],true,dl);
		if( frac[i] <= 0. )
		{
						 ...
						 /*@out@*/ sd_data *sd)
{
	...
	/* first read magic number */
	mie_next_data(chFile,io2,chLine,&dl);
	mie_read_number(chFile,chLine,&sd->magic,true,dl);
	...
	case SD_SINGLE_SIZE:
		/* single sized grain */
		mie_next_data(chFile,io2,chLine,&dl);
		mie_read_number(chFile,chLine,&sd->a[ipSize],true,dl);
	...
	case SD_NR_CARBON:
		...
		mie_next_data(chFile,io2,chLine,&dl);
		mie_read_number(chFile,chLine,&sd->nCarbon,true,dl);
		break;
	case SD_POWERLAW:
		/* simple power law distribution, first get lower limit */
		mie_next_data(chFile,io2,chLine,&dl);
		mie_read_number(chFile,chLine,&sd->a[ipBLo],true,dl);
		...
		/* upper limit */
		mie_next_data(chFile,io2,chLine,&dl);
		mie_read_number(chFile,chLine,&sd->a[ipBHi],true,dl);
	...
	case SD_EXP_CUTOFF3:
		...
		mie_next_data(chFile,io2,chLine,&dl);
		mie_read_number(chFile,chLine,&sd->a[ipBLo],false,dl);

		/* "upper" limit, above this the exponential cutoff sets in */
		mie_next_data(chFile,io2,chLine,&dl);
		mie_read_number(chFile,chLine,&sd->a[ipBHi],false,dl);
		...
		mie_next_data(chFile,io2,chLine,&dl);
		mie_read_number(chFile,chLine,&sd->a[ipSLo],false,dl);

		/* scale size for upper exponential cutoff, zero indicates normal cutoff */
		mie_next_data(chFile,io2,chLine,&dl);
		mie_read_number(chFile,chLine,&sd->a[ipSHi],false,dl);
	...
	case SD_LOG_NORMAL:
		...
		mie_next_data(chFile,io2,chLine,&dl);
		mie_read_number(chFile,chLine,&sd->a[ipGCen],true,dl);

		/* 1-sigma width */
		mie_next_data(chFile,io2,chLine,&dl);
		mie_read_number(chFile,chLine,&sd->a[ipGSig],true,dl);
	...
	case SD_TABLE:
		...
		mie_next_data(chFile,io2,chLine,&dl);
		mie_read_number(chFile,chLine,&sd->npts,true,dl);
		if( sd->npts < 2 )
		{
	...
	if( lgTryOverride )
	{
		...
		mie_next_data(chFile,io2,chLine,&dl);
		mie_read_number(chFile,chLine,&help,false,dl);
