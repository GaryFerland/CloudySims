		  ...
		  /*@in@*/const GrainPar& gp)
{
	...
	io2 = open_data( chFile, "r", AS_LOCAL_DATA );

	/* include the name of the file we are reading in the Cloudy output */
	string chLine2;
	size_t flen = strlen(chFile);
	if( flen <= 40 )
	{
		chLine2 = string(chFile) + string(40-flen, ' ');
	}
	else
	{
		chLine2 = string(chFile).substr(0, 37) + "...";
	}
	if( called.lgTalk )
