...
public:
	...
	double operator() (double EOverKT) const
	{
		...
		double col_str = collision_strength_VF01<P>( ipISO, EOverKT * temp / TE1RYD,
																*this );
...
void HeCollidSetup( void )
{
	...
	static const int chLine_LENGTH = 1000;
					...
					double Aul, double tauLo, double EnerWN, double EnerErg, const char **where )
{
	...
	*where = "      ";
	...
	else if( (nHi==nLo) && (sHi==sLo) )
	{
		...
		if( iso_ctrl.lgCS_Seaton[ipHE_LIKE] && Collider == ipELECTRON )
		{
			...
			/* FG 01/10/2020: Seaton does not deal well with protons, and so it is degraded to deal
			 * with only electrons. The default for l<3 is now PSM.
			 */
		...
		else if( abs(lHi-lLo)==1 && iso_ctrl.lgCS_PS64[ipHE_LIKE])
		{
			...
			/* statistical weights included */
			/*FG 01/10/2020: PS64 and PSM deal with l<3 by default */
	...
	/* n-changing collision with no quantal calculations */
	else if( nHi != nLo )
	{
		if( iso_ctrl.lgCS_Vriens[ipHE_LIKE] )
		{
			...
			double Aul_j = HydroEinstA(nHi,nLo);
			cs = (realnum)CS_VS80( nHi, gHi, IP_Ryd_Hi, nLo, gLo, IP_Ryd_Lo, Aul_j, nelem, Collider, phycon.te );
			*where = "Vriens";
			if( nelem != ipHELIUM )
			{
				fixit( "This should not be set here!" );
				factor1 = 1.f;
			}
		}
		...
		else if( nelem != ipHELIUM )
		{
			...
			/* this branch is for testing and reached with command SPECIES HE-LIKE COLLISIONS VRIENS OFF */
			...
			long nLo, long lLo, long sLo, long jLo )
{
	...
	long ipLo = iso_sp[ipHE_LIKE][nelem].QuantumNumbers2Index[nLo][lLo][sLo];
	long ipHi = iso_sp[ipHE_LIKE][nelem].QuantumNumbers2Index[nHi][lHi][sHi];
