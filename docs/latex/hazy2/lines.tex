...
\subsubsection{PAH bands in \cdFilename{continuum\_bands.ini}}

A number of entries with labels ``PAH'' and ``PAHC'' appear in Table\ref{tab:continuum_bands}.
The entries with the label ``PAH'' are centered on the PAH feature. 
Those labeled `PAHC'' are regions near the PAH feature chosen to have the same bandwidth
as the PAH feature. 
The ``PAH'' entry is the total emission including the underlying continuum, and perhaps 
line emission. 
The ``PAHC'' entries to either side of the ``PAH'' entry so that that net PAH feature could be
approximated at the difference between the ```PAH'' feature minus the ``PAHC'' continuum.
...
By default, the Fe~{\sc ii} bands defined in \cdFilename{data/FeII\_bands.ini},
