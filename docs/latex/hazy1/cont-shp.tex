...
\noindent
will search for the file \cdFilename{cf.sed}, 
first in the current directory, and then in the
\cdCommand{SED} folder of the \cdCommand{data} directory.  
...
the \cdCommand{units} option and one of the units given
...
\cdCommand{save transmitted continuum} command described on page
...
star available} after the grids have been installed and compiled. The 
...
repeated with the \cdCommand{table star available} command in order
to see the binary files. The output will also list the \cdCommand{table HM05}
...
grid, you can use the \cdCommand{table star $<$grid$>$ list} command. This
