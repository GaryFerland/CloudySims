...
\subsection{Table SED available}

The primary documentation for the SED files included in the distribution
has been added to the set of files in the \cdFilename{data/SED} directory.
Consult the \cdFilename{ReadMe.txt} file to find out more about what is available.

You can also obtain a list of all the SED files that are present on the search
path using the \cdCommand{table sed available} command. This will include
user-installed SED files by simply listing all files with names ending with \cdFilename{.sed}.
It is not possible to check whether these files actually contain a valid SED table.
...
grid infrastructure to do their work. This command will exit immediately after printing the list.
...
\begin{verbatim}
compile star "merged_file.ascii" # optional
\end{verbatim}
and used as follows
\begin{verbatim}
