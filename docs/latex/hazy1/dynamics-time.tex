...
hydrogen-ionizing photons varies with time.  Right after the \cdCommand{time} command, a constant time step of $10^7$~s is used and the stopping time is also determined. These are followed by a series of lines giving the
...
with \cdCommand{age} but these characters are ignored. They are only present to improve readability.
...
starting with \cdCommand{end}. The dynamical calculations will be terminated when one of these three conditions happens:
1- all of the determined iterations are completed.
2- the stop time defined by the \cdCommand{time} command is reached.
...
\begin{verbatim}
# set the lasers, their brightness  and the distance
laser 1 Ryd
