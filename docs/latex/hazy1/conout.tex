...
used as a directory separator. This command will exit immediately after printing
the list.
...
or luminosity case is used. See Section~\ref{sec:IntensityLuminosityCases} for a definition of these cases.
In the intensity
...
Beginning in C17 output is in true luminosity units, $\nu L_\nu$, in the luminosity case.
...
This saves the total two-photon continuum emitted in the last zone of the model
and is mainly a debugging
...
\subsection{Save grain temperature [ average ]}

This command has two output modes. The default mode will print multiple lines
per zone. The first line for each zone will start with the depth (in cm)
followed by the label of the first grain size bin. The remainder of the line
will show pairs of numbers giving the charge of the grain (in elementary
charge units) and the temperature of the grain (in K). Such pairs will be
shown for every charge state that is modeled by the code. Subsequent lines
will give similar output for the remaining grain size bins, except that the
depth of the zone will not be repeated and is replaced by a field of spaces
for easy readability.

If the keyword \cdCommand{average} is included on the command line, a
different output mode will be used that is very similar to the output mode
that was the default in \Cloudy\ versions C25 and before. Here only one line
of output will be produced for every zone. This line starts with the depth of
the zone (in cm) followed by a weighted average of the grain temperature over
all charge states (in K) for all size bins. The advantage of this mode is that
it is easier to plot, but it has the disadvantage that it does not output the
actual grain temperatures, only a weighted average over all charge states
which has no strict physical meaning.
...
depth [cm] into the cloud.  The remaining columns give the optical depth for each line.  Results are reported for the last computed zone unless the \textbf{every} option appears.

The following examples illustrate its use for reporting either the final (last-zone) result or values at every zone, respectively:

\begin{verbatim}
save line optical depth some "lines.opt"
o  3 5006.84A
o  1 6300.30A
H  1 4340.46A
H  1 4861.32A
...
\begin{verbatim}
save line optical depth some "lines.opte" every
o  3 5006.84A
o  1 6300.30A
H  1 4340.46A
H  1 4861.32A
...
\cdCommand{convergence}&top level convergence \\
\hline
\end{tabular}
\end{table}
