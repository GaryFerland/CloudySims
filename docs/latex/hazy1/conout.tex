...
\section{Save hyperfine populations
        [row [lower$\vert$upper$\vert$ratio$\vert$tspin]]}
\label{sec:CommandSaveHyperfinePops}

This saves the upper and lower level populations of the requested hyperfine lines.
The command expects two filenames to be given, each enclosed in double quotes.
The first specifies the output file, while the second contains the hyperfine
lines to be tracked.
For example,
%
\begin{verbatim}
save hyperfine populations "output.txt" "linelist.dat"
\end{verbatim}
%
will read lines from \cdFilename{linelist.dat} and save the output in
\cdFilename{output.txt}.

By default, there is one line of output per hyperfine line per zone.
...
\end{verbatim}
%
If the \cdCommand{row} keyword appears, the command will generate one output
line for each zone, listing the depth, and for each spectral line, one of the
columns above.
By default, the population ratio is reported, but the lower level, upper level
populations, or the spin temperature may be reported, instead, with the
keywords \cdCommand{lower}, \cdCommand{upper}, and \cdCommand{tspin},
respectively; by default, \cdCommand{ratio} is used is used.
...
\end{verbatim}
%
The population ratio, $(n_u/g_u)/(n_l/g_l)$, may be used by external programs
to correct a given absorption coefficient for stimulated emission.
