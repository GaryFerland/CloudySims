...
\cdCommand{database H-like collisions l-mixing Pengelly [Classic/PSM]} 
The  modified \citet{PengellySeaton1964} formalism, PS-M, described by 
\citep{Badnell2021}, is used by default.
If the key \cdCommand{Classic} is used, then the original \citet{PengellySeaton1964} is used.
If the key \cdCommand{PSM} is used, then the slightly different modified theory from \citep{Guzman.II.2017}
is used.
...
criteria is followed. If no command, standard calculations from \citep{Badnell2021}, with the criteria used in \citet{Guzman.II.2017}, 
will be performed. The \cdCommand{options} are either \cdCommand{Pengelly}, 
\cdCommand{Vrinceanu}, \cdCommand{VOS12}, or \cdCommand{PSM} (see above).
