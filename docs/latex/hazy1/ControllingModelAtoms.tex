...
This option specifies LTE level populations will be used for 
...
of the code, so the simple \htwo\ models outlined above were used by default. 
Processors were not powerful enough to routinely employ the full
model of \htwo.
...
The keyword \cdCommand{large} specifies a preset size of 40 resolved and 160
...
The keyword \cdCommand{small} specifies a preset size of 5 resolved levels
...
Use the \cdCommand{database xx-like levels print} command to confirm that you are using
...
The nP to 1s lines of the H-like isoelectronic sequence are resolved into the
...
This command does not affect the model atoms for \Ion{H}{I} and \Ion{He}{II}.
...
of the available energy levels.
See \citet{2013MNRAS.429.3133L} for more details.
Beware that using the \cdCommand{max} option (especially when combined with
the \cdCommand{mixed} or \cdCommand{theoretical} options described below)
will result in models that require a considerable amount of CPU time and memory.

\subsection{Database Chianti [experimental, theoretical, mixed]}
\label{sec:AtomChiantiExperimental}
By default we use only experimental Chianti energies.
The \cdCommand{experimental} option also specifies this choice.
The \cdCommand{theoretical} option causes \Cloudy\ to only use only theoretical 
energies.
This provides significantly more levels and transitions, 
however the uncertainty in wavelength is larger.
In rare cases where the theoretical energy is missing, but the experimental energy
is provided, the latter will be used.
The \cdCommand{mixed} option causes \Cloudy\ to use experimental where
