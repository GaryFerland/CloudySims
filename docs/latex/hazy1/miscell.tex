...
In some cases, the stellar atmosphere
...
for the \cdCommand{grains} command to function. These are already included
in the distribution, so normally there is no need for users to run this
command without options. The options are described below.
Examine the output to check
for problems. 

The opacity file created by this command needs to be installed somewhere
on the search path (see Section~\ref{sec:ReadingWritingFiles}), usually either
the data directory or the local directory where you are running the code.

The simplest way to execute this command is as follows:\\
1) \texttt{cd} into the directory where you want to install the opacity file\\
2) Then run \Cloudy\ as follows:
\begin{verbatim}
~/path to source/cloudy.exe  -e  'compile grains [options]'
\end{verbatim}
...
\item 
PAHs are much smaller than regular grains and must be paired with a
matching size distribution, like e.g. \texttt{ab08}, \texttt{c15}, or \texttt{c120}, rather than Orion or ISM.
\end{itemize}
...
\cdFilename{continuum\_mesh.dat}, which is located in the data directory.  Permanent
...
the file \cdFilename{continuum\_mesh.dat}. This assures energy
