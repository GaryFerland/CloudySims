...
This section discusses the default geometry considered by \Cloudy.
The special case of a cylinder geometry is covered further down.
The \cdTerm{covering factor} is the fraction of $4\pi$~sr
covered by gas when viewed
...
Users can set up a special geometry using the \cdCommand{cylinder} command
(see Section~\ref{cmd:cylinder}). In this setup the gas will not cover the
full $4\pi$~sr when viewed from the center of the nebula. However, since all
the necessary information is already specified with the \cdCommand{cylinder}
command, this is automatically taken into account and normally no further
action is needed.

It is however possible to combine the \cdCommand{cylinder} command with a
\cdCommand{covering factor} command. This would then indicate that the
cylinder is incomplete. In that case the covering factor would be defined as
the fraction of $\Omega$~sr that is actually covered by gas, where $\Omega$
is the solid angle that would be covered by a full cylinder.
...
The code allows the user to define zero, one, or even multiple external radiation fields.
An external radiation field is usually specified with
...
There are two ways to specify the brightness of an external radiation field.

\subsection{Luminosity case}

If the brightness of at least one of the external radiation fields is specified as a ``luminosity'',
the light radiated by the central object into $4\pi$~sr, then the luminosity case will be in effect.
...
If there is no external radiation field, or if all external radiation fields are specified as an intensity,
then the intensity case will be in effect.
