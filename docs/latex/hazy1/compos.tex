...
\subsection{Metals deplete commands}

These commands specify depletion factors that account for elements that
have condensed onto grains.  
The standard approach is to define the gas-phase abundances as
\begin{equation}
A( \textrm{gas phase}) = A( \textrm{reference}) \times  \textrm{Depletion}
\end{equation}
where the reference abundance is the composition the user specifies with other commands
and the depletion is set with these commands.
The final gas-phase abundance results.
There are other commands that specify the gas-phase abundances directly.
...
We include solid grains so suffer from the problem pointed out by \citet{Snow1996}.


\subsubsection{metals deplete --- table of gas-phase depletion factors}
...
\subsubsection{metals deplete Jenkins 2009 Fstar=0.5}
...
is related to $F_*$ by his equation 10, the coefficients listed in Table 4, 
and and the special case of S from Section 9 of his paper.
Our version of his table is stored as the file \cdFilename{Jenkins09\_ISM\_Tab4.dep} 
in the \cdFilename{abundances} directory.  

We implement this as follows.
First you specify the composition with other commands such as the
\cdCommand{abundance} command described in Section \ref{sec:CommandAbundances}.
We  modify those abundances by
the \citet{2009ApJ...700.1299J}  depletion factors, as described by
Gunasekera+2021 (in preparation).  
The depletion factor $D_X$ of an element $X$ is given by
\begin{equation}
D_X = 10^{B_X + A_X (F_* - z_X)}
\end{equation}
where the coefficients come from Jenkins' tables.
This factor multiplies the previously-specified gas-phase abundance of $X$
to set the gas-phase abundance used in the calculation, as in
\begin{equation}
\left[ \frac{n(X)}{n(H)} \right]_{gas} = D_X \times \left[\frac{n(X)}{n(H)} \right]_{specified}
\end{equation}
The full command syntax is
\begin{verbatim}
metals deplete Jenkins 2009 Fstar=0.5 [vary]
\end{verbatim}
...
\cdCommand{grains}  command.
The \cdCommand{grains}  command is described in section \ref{sec:GrainsCommand}.
As described in the previous \cdCommand{metals deplete} command, we do not 
...
The \citet{2009ApJ...700.1299J}   ISM abundances could be approximated by 
\begin{verbatim}
abundances "solar_Lodders09.abn"
metals deplete Jenkins 2009 Fstar=0.5
grains
\end{verbatim}
using the composition stored in the \cdFilename{solar\_Lodders09.abn} file 
in the \cdFilename{abundances} directory.
...
allowing the \cdCommand{grid} command
 ...
 The \cdCommand{print} option will vary Fstar between 0 and 1 and report the depletion factors.
