...
\cdCommand{fortnights}, \cdCommand{months} (defined as 1/12 of a year),
\cdCommand{years} (we adopt the Julian year, or 365.25 days, as our definition
of the year), \cdCommand{centuries}, \cdCommand{millennia}, \cdCommand{Myr},
and \cdCommand{Gyr} change the units.
...
In the default geometry this sets a covering factor $\Omega/4\pi$ for the emission-line region
...
It is impossible to specify a covering factor of zero. The interaction of
this command with the \cdCommand{cylinder} command described below is
discussed in Section~\ref{cov_cylinder}.
